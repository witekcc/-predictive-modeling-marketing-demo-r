---
title: "Building classifiers for advertisement campaigns"
---
set data path:
```{r cache=TRUE}
path <- "/home/witek/projects/ml/da-emas-banners-r/"
setwd(path) 
```
...
```
Some of the categories have more then 52 levels; randomForest function can't handle them; example:
...
output: <br>
models - vektor of M tree classifiers
```{r cache=TRUE}
...
```{r cache=TRUE}
require("stringr")
testit<- function(M = 500,n = 35, neg_test = 1000, allmodelsRDS = "allmodels.rda", 
                  testowyRDS = "testowy.rda", predictionsRDS = "predictions.rda",
                  logRDS = "log.rda"){
  ...
  predictions<- numeric(nrow(testowy))
  for(i in 1:nrow(testowy)){
    
    predictions[i]<- which.max(classify_observations(testowy[i,], allmodels))
    saveRDS(predictions, file = predictionsRDS)
    saveRDS((sort( sapply(ls(),function(x){object.size(get(x))}))),file = logRDS)
    
  }
  result_table <- table(predictions,testowy$ad_id)
  return(result_table)
}

```
Function testing two classificators<br>
<br>
input:<br>
k1file - classifier 1<br>
k2file - classifier 2<br>
...
daneRDS - data
```{r cache=TRUE}
testit_3<-function(k1file = "am2.rda", k2file = "am2_1.rda", pred1file = "pred1.rda",
...
**am4.rda** <- classifier consisting of 50 trees for every campaign using 35 yes and 35 no observations
